...
class cliente {
    ...
    try {
      if (_id == "" || _id == undefined) {
        ...
        if (item == "" || item == undefined) {
          jsonRetorno.status = 200
          jsonRetorno.json = { status: false, descricao: "cliente não encontrado!" }
        } else {
          jsonRetorno.status = 200
  ...
  static async delete(_id) {
    ...
    try {
      ...
      jsonRetorno.status = 200
      jsonRetorno.json = { status: true, descricao: "cliente deletado com sucesso!", item: item }
    } catch (error) {
      jsonRetorno.status = 200
      jsonRetorno.json = { status: false, descricao: error.toString() }
    }
  ...
  static async post(body) {
    ...
    try {
      ...
      jsonRetorno.json = { status: true, descricao: "cliente criado com sucesso!", item: itemCriado }
    } catch (error) {
      jsonRetorno.status = 200
      jsonRetorno.json = { status: false, descricao: error.toString() }
    }
  ...
  static async put(body) {
    ...
    try {
      ...
      jsonRetorno.status = 200
      jsonRetorno.json = { status: true, descricao: "cliente atualizado com sucesso!", item: itemAtualizado }
    } catch (error) {
      jsonRetorno.status = 200
      jsonRetorno.json = { status: false, descricao: error.toString() }
    }
